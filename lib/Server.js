class Server {
  ...
  constructor ({ schema, baseURL, logger = console }) {
    // Logger
    this.logger = logger
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      const gracefulShutdown = (server, signal) => {
        this.logger.info(`Received kill signal (${signal}), shutting down gracefully.`)
        server.close(() => {
      ...
      this.server = this._app.listen(port, err => {
        ...
        this.logger.info(`Express  : ${this.baseURL}`)
        resolve(this.server)
      })
  ...
  stop () {
    ...
    return new Promise((resolve, reject) => {
      this.logger.info(`Express  : closing...`)
      const _server = this.server
      this.server = null
      _server.close(() => {
        this.logger.info(`Express  : bye!`)
        resolve(true)
        process.exit()
      })
    })
